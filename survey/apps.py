...
class AppConfig(DjangoApponfig):
    ...
    def ready(self):
        ...
        if (self.current_survey_schedule and
                self.current_survey_schedule not in site_surveys.get_survey_schedule_field_values()):
            raise SurveyError(
                'Invalid current survey schedule specified. See AppConfig. Got {}.'
                'Expected one of {}.'.format(site_surveys.get_survey_schedule_field_values()))
        else:
            sys.stdout.write(' * current survey schedule : {}\n'.format(self.current_survey_schedule or '<not set>'))
